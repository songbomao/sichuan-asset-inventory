/**
 * 🔥🔥 HOTFIX v202607301556 - React error #300 hooks 顺序真因修复（GlobalAppBar / AdminTasks early return 跳 hook）🔥🔥
 * 应用版本信息
 * 每次发布前更新：
...
export const VERSION_HISTORY: Array<{
  ...
  },
  {
    ...
    notes: '重构拍照流程为三步骤引导拍摄——①标签照(校验标签内容)②正面照(校验实物轮廓)③反面照(校验实物轮廓)，每步拍摄后实时校验照片内容是否匹配，校验不通过引导重拍，校验通过自动进入下一步。废除原有自动二维码分类逻辑，改为人工引导三步采集。',
  },
  {
  ...
  },
  {
    ...
    notes: '重新设计拍照快门交互：移除取消按钮下方的异常方块快门，改为底部居中 native 相机风格大圆快门按钮（白边+白心+按下缩放反馈），左下角取消，右下角显示已拍数量，整体风格统一直观。',
  },
  {
  ...
  },
  {
    ...
    notes: '后端ZXing.Net二维码解码兜底：前端jsQR多尺度解码失败后，自动调用后端DecodeQr接口（免登录白名单），后端ZXing.Net多尺度（原始→1.5x→2x→0.5x）解码，双重兜底提升扫码成功率。',
  },
  {
  ...
  },
  {
    ...
    notes: '盘点页视觉重构+二维码兜底：①顶部4步引导条、资产信息可折叠、照片/AI/扫码合并为单卡、结果填为单卡、底部提交未拍完禁用；②钉钉原生扫码+手动标记二维码弹窗+照片缩略图标记按钮，三重兜底。',
  },
  {
  ...
  },
  {
    ...
    notes: '彻底修复 React error #300：GlobalAppBar 与 AdminTasks 的 early return 位于 hooks 之前导致 hooks 数量不一致，移至全部 hooks 之后；AST 全仓扫描确认无其它同类违规。',
  },
  {
  ...
  },
  {
    ...
    notes: '资产详情字段精简 + AI 识别按钮常驻：①砍掉 30 个冗余字段保留 18 个核心字段重新分 3 组（基本信息/位置信息/财务与使用）；②AI 识别按钮常驻，未拍照时禁用。',
  },
  {
  ...
  },
  {
    ...
    notes: '新建盘点任务两个选择器默认全选并增强责任人选择器：①资产预览弹窗打开时若当前无已选资产，自动全选全部资产（与底部「全选」按钮同逻辑，总量大时分批拉取）；②选择盘点责任人弹窗在人员列表加载后若当前无已选责任人，自动全选当前部门范围全部人员；③在责任人选择器右侧列表上方新增「全选」「取消全选」按钮，即时更新选中状态。默认全选仅在当前选择为空时触发，避免覆盖用户已有精确选择。',
  },
  {
    version: 'v202607291614',
    time: '2026-07-29 16:14',
    notes: '修复三处UI：①全局进度页(/admin/dashboard)顶部间距过大——该页在Layout内，Layout<main>已提供pt-12，原Dashboard自身再叠加pt-12造成双重顶距(96px)，现按isGlobal条件去除Layout内页pt-12，与管理页一致；②导航栏直角与卡片圆角割裂——GlobalAppBar底部加12px圆角(borderRadius:0 0 12px 12px)与全局MuiCard统一；③返回控制台点击无效——handleConsoleClick目标页判定由单一isAdmin改为「isAdmin或当前在/admin/*路径下」双判据，消除isAdmin异步就绪前错判为责任人首页导致跳转失效的死点(ConsoleButton等浮动按钮为未渲染死代码，非根因)。',
  },
  {
    version: 'v202607291535',
    time: '2026-07-29 15:35',
    notes: '导航栏配色统一为全局主题渐变：由白底紫字改为深靛蓝→紫 linear-gradient(135deg,#1a237e→#4a148c) 背景 + 白色文字，与页面 containedPrimary 主按钮（同款渐变）及科技感紫调一致；悬停/激活态用半透明白叠加(rgba(255,255,255,.16/.28))，移除灰色底边框改为主题色投影。',
  },
  {
    version: 'v202607291529',
    time: '2026-07-29 15:29',
    notes: '选择盘点责任人弹窗与资产预览弹窗宽度对齐主容器：全屏 Dialog 的 Paper 限制 max-width 480px 并水平居中，与 #root 移动端容器及系统顶栏保持同宽，消除宽屏下左右撑破的视觉割裂。',
  },
  {
  ...
  },
  {
    ...
    notes: '修复系统顶栏与下方内容宽度不一致：AppBar 由视口满宽改为 max-width 480px 水平居中（与 #root 移动端容器一致），左右边距与页面 p-4 对齐，顶栏不再比下方内容宽。',
  },
  {
    version: 'v202607291403',
    time: '2026-07-29 14:03',
    notes: '责任人端「我的盘点任务」卡片截止时间展示时分秒：formatDeadline 由仅显示月日扩展为「MM/DD HH:mm:ss」，保留今日截止/剩余N天/已过期等语义前缀。',
  },
  {
    version: 'v202607291401',
    time: '2026-07-29 14:01',
    notes: '返回控制台按钮重构：由漂浮圆形图标改为固定在页面顶部的系统级顶栏（左侧「蜀资点兵」品牌、右侧「返回控制台」文字按钮，replace 落地角色首页），彻底消除游离于系统之外的视觉割裂感；所有页面（含独立页盘点/详情/复盘/报告/看板）均固定可见，点击可靠返回控制台。',
  },
  {
  ...
  },
  {
    ...
    notes: '资产对比同步 Tab 改为单行 nowrap 等宽布局：4 个差异分类始终排在同一行、间距均匀、移动端不折行；Tab 文字精简为「仅SAP/仅本地/不一致/责任人」。功能逻辑与异常自动选中不变。',
  },
  {
