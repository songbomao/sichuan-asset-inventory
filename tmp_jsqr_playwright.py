"""Test jsQR on user's photo in real browser using Playwright"""
import base64, io, json, sys
from PIL import Image
from playwright.sync_api import sync_playwright

# Load photo, simulate CameraCapture pipeline
img = Image.open(r"D:\Program Files\Tencent\xwechat_files\sbm263696893_34b9\temp\RWTemp\2026-07\88df94ef9d6efa54a52811fd3f5c14ba.jpg")
w, h = img.size
scale = min(1.0, 1200 / max(w, h))
bw, bh = int(w * scale), int(h * scale)
small = img.resize((bw, bh), Image.LANCZOS)
buf = io.BytesIO()
small.save(buf, format='JPEG', quality=92)
buf.seek(0)
dataUrl = "data:image/jpeg;base64," + base64.b64encode(buf.read()).decode()

print(f"Photo: {bw}x{bh}, dataUrl len: {len(dataUrl)}")

# Find the jsQR module path
jsqr_path = r"D:\work\workspace\workBuddy\sichuan-asset-inventory\sai-inventory-h5\sichuan-asset-inventory\node_modules\jsqr\dist\jsqr.js"

with sync_playwright() as p:
    browser = p.chromium.launch(headless=True)
    page = browser.new_page()

    # Inject jsQR as a script tag
    html = f"""<!DOCTYPE html>
<html><body style="margin:20px;font-family:monospace;white-space:pre">
<canvas id="c" style="display:none"></canvas>
<div id="out" style="white-space:pre"></div>
<script>
const out = document.getElementById('out');
function log(s) {{ out.textContent += s + '\\n'; console.log(s); }}

// Load jsQR
const jsqrScript = document.createElement('script');
jsqrScript.src = 'file:///{jsqr_path.replace(chr(92), '/')}';
jsqrScript.onload = () => {{
  log('jsQR loaded');
  const img = new Image();
  img.onload = () => {{
    log(`img loaded: ${{img.width}}x${{img.height}}`);
    const canvas = document.getElementById('c');
    const ctx = canvas.getContext('2d');

    // Test 1: full image
    canvas.width = img.width;
    canvas.height = img.height;
    ctx.drawImage(img, 0, 0);
    let raw = ctx.getImageData(0, 0, img.width, img.height);
    let res = jsQR(raw.data, raw.width, raw.height);
    log(`full ${{img.width}}x${{img.height}}: ${{res ? res.data : 'null'}}`);

    // Test 2: grayscale enhancement
    const px = raw.data;
    let minGray = 255, maxGray = 0;
    const gray = new Uint8Array(img.width * img.height);
    for (let i = 0, j = 0; i < px.length; i += 4, j++) {{
      const g = px[i] * 0.299 + px[i + 1] * 0.587 + px[i + 2] * 0.114;
      gray[j] = g;
      if (g < minGray) minGray = g;
      if (g > maxGray) maxGray = g;
    }}
    const lo = Math.max(minGray, Math.round(minGray + (maxGray - minGray) * 0.02));
    const hi = Math.min(maxGray, Math.round(maxGray - (maxGray - minGray) * 0.02));
    const range = hi > lo ? 255 / (hi - lo) : 1;
    for (let i = 0, j = 0; i < px.length; i += 4, j++) {{
      const v = Math.round(Math.max(0, Math.min(255, (gray[j] - lo) * range)));
      px[i] = v;
      px[i + 1] = v;
      px[i + 2] = v;
    }}
    ctx.putImageData(raw, 0, 0);
    res = jsQR(raw.data, raw.width, raw.height);
    log(`grayscale enhanced: ${{res ? res.data : 'null'}}`);

    // Test 3: center crop bottom 65%
    ctx.drawImage(img, 0, 0);
    const cropY = Math.round(img.height * 0.35);
    const cropH = Math.round(img.height * 0.65);
    canvas.width = img.width;
    canvas.height = cropH;
    ctx.drawImage(img, 0, cropY, img.width, cropH, 0, 0, img.width, cropH);
    raw = ctx.getImageData(0, 0, img.width, cropH);
    res = jsQR(raw.data, raw.width, raw.height);
    log(`crop bottom 65%: ${{res ? res.data : 'null'}}`);

    // Test 4: crop bottom 50%
    ctx.drawImage(img, 0, 0);
    const cropY2 = Math.round(img.height * 0.5);
    const cropH2 = Math.round(img.height * 0.5);
    canvas.width = img.width;
    canvas.height = cropH2;
    ctx.drawImage(img, 0, cropY2, img.width, cropH2, 0, 0, img.width, cropH2);
    raw = ctx.getImageData(0, 0, img.width, cropH2);
    res = jsQR(raw.data, raw.width, raw.height);
    log(`crop bottom 50%: ${{res ? res.data : 'null'}}`);

    // Test 5: crop very bottom 30%
    ctx.drawImage(img, 0, 0);
    const cropY3 = Math.round(img.height * 0.7);
    const cropH3 = Math.round(img.height * 0.3);
    canvas.width = img.width;
    canvas.height = cropH3;
    ctx.drawImage(img, 0, cropY3, img.width, cropH3, 0, 0, img.width, cropH3);
    raw = ctx.getImageData(0, 0, img.width, cropH3);
    res = jsQR(raw.data, raw.width, raw.height);
    log(`crop bottom 30%: ${{res ? res.data : 'null'}}`);

    // Test 6: multi-scale
    for (const s of [0.8, 0.65, 0.5, 0.35, 0.25]) {{
      canvas.width = Math.round(img.width * s);
      canvas.height = Math.round(img.height * s);
      ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
      raw = ctx.getImageData(0, 0, canvas.width, canvas.height);
      res = jsQR(raw.data, raw.width, raw.height);
      log(`scale ${{s}}: ${{res ? res.data : 'null'}}`);
      if (res) break;
    }}

    // Test 7: inverted
    ctx.drawImage(img, 0, 0, img.width, img.height);
    canvas.width = img.width;
    canvas.height = img.height;
    raw = ctx.getImageData(0, 0, img.width, img.height);
    for (let i = 0; i < raw.data.length; i += 4) {{
      raw.data[i] = 255 - raw.data[i];
      raw.data[i+1] = 255 - raw.data[i+1];
      raw.data[i+2] = 255 - raw.data[i+2];
    }}
    res = jsQR(raw.data, raw.width, raw.height);
    log(`inverted: ${{res ? res.data : 'null'}}`);

    // Test 8: binary threshold
    ctx.drawImage(img, 0, 0, img.width, img.height);
    raw = ctx.getImageData(0, 0, img.width, img.height);
    let sum = 0;
    for (let i = 0; i < raw.data.length; i += 4) {{
      sum += raw.data[i] * 0.299 + raw.data[i+1] * 0.587 + raw.data[i+2] * 0.114;
    }}
    const avg = sum / (img.width * img.height);
    log(`avg gray: ${{avg.toFixed(0)}}`);
    for (let i = 0; i < raw.data.length; i += 4) {{
      const g = raw.data[i] * 0.299 + raw.data[i+1] * 0.587 + raw.data[i+2] * 0.114;
      const v = g < avg * 0.7 ? 0 : 255;
      raw.data[i] = v;
      raw.data[i+1] = v;
      raw.data[i+2] = v;
    }}
    res = jsQR(raw.data, raw.width, raw.height);
    log(`binary threshold: ${{res ? res.data : 'null'}}`);

    // Test 9: Check if the image is actually what we expect
    // Draw a 100x100 crop from the bottom area to see if QR is visible
    ctx.drawImage(img, 0, 0);
    canvas.width = 100;
    canvas.height = 100;
    ctx.drawImage(img,
      Math.round(img.width * 0.3), Math.round(img.height * 0.7),
      100, 100,
      0, 0, 100, 100);
    raw = ctx.getImageData(0, 0, 100, 100);
    let darkCount = 0;
    for (let i = 0; i < raw.data.length; i += 4) {{
      const g = raw.data[i] * 0.299 + raw.data[i+1] * 0.587 + raw.data[i+2] * 0.114;
      if (g < 128) darkCount++;
    }}
    log(`bottom area 100x100 dark ratio: ${{(darkCount/10000*100).toFixed(1)}}%`);

    log('=== ALL DONE ===');
  }};
  img.src = '{dataUrl}';
}};
document.head.appendChild(jsqrScript);
</script>
</body></html>"""

    page.set_content(html)
    page.on('console', lambda msg: print(f'[BROWSER] {msg.text}') if msg.type != 'log' else None)
    page.on('pageerror', lambda err: print(f'[PAGE ERROR] {err}'))
    page.wait_for_selector('#out', state='attached', timeout=15000)
    page.wait_for_function("document.getElementById('out').textContent.includes('ALL DONE')", timeout=30000)
    result = page.evaluate("document.getElementById('out').textContent")
    print(result)
    browser.close()